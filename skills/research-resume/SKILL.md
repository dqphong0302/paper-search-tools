...
description: Resume research from ScholarGateway's interested papers and notes; when its local server is unavailable, continue only from user-supplied context with standard web search.
---

# Resume research

Read the current ScholarGateway MCP schemas, then use the single interest library.
...
- Web fallback is read-only with respect to ScholarGateway. Any save or note update remains pending until the server is reachable and the mutation succeeds.
