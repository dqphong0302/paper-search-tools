...
fn rate_check(key: &str, limit: u32) -> Result<(), u64> {
    ...
    let Ok(mut windows) = state.lock() else { return Ok(()) };
...
pub fn trusted_origin(origin: &HeaderValue, port: u16) -> bool {
    origin.to_str().ok().is_some_and(|origin| UI_ORIGINS.contains(&origin)
...
pub async fn guard(
    ...
    let authority = headers.get("host").and_then(|value| value.to_str().ok())
        .or_else(|| request.uri().authority().map(|value| value.as_str()));
    let Some(authority) = authority else { return StatusCode::BAD_REQUEST.into_response(); };
    let parsed = authority.parse::<axum::http::uri::Authority>();
    let Ok(authority) = parsed else { return StatusCode::BAD_REQUEST.into_response(); };
    if !["localhost", "127.0.0.1", "[::1]"].contains(&authority.host()) {
        return StatusCode::FORBIDDEN.into_response();
    }
    let port = authority.port_u16().unwrap_or(80);
    if let Some(origin) = headers.get("origin") {
        if !trusted_origin(origin, port) { return StatusCode::FORBIDDEN.into_response(); }
    } else if headers.get("sec-fetch-site").is_some_and(|value| value != "same-origin" && value != "none") {
    ...
    if !is_public(request.method(), request.uri().path()) {
        ...
        if request.uri().path().starts_with("/api/agents") && !matches!(principal, crate::agents::Principal::Admin) {
        ...
        if limit > 0 {
            ...
            if let Err(retry) = rate_check(&key, limit) {
                ...
                response.headers_mut().insert("retry-after", HeaderValue::from(retry));
                return response;
            }
        }
    }
...
async fn scope_rest(state: &crate::server::AppState, grant: &crate::agents::Grant, request: Request) -> Result<Request, StatusCode> {
    use crate::agents::{paper_allowed, workspace_allowed};
    let path = urlencoding::decode(request.uri().path()).map_err(|_| StatusCode::BAD_REQUEST)?.into_owned();
    let method = request.method().clone();
    let parts: Vec<_> = path.split('/').collect();
    let url = reqwest::Url::parse(&format!("http://localhost{}", request.uri())).map_err(|_| StatusCode::BAD_REQUEST)?;
    let query = |key: &str| -> Result<String, StatusCode> {
        let values: Vec<_> = url.query_pairs().filter(|(k, _)| k == key).map(|(_, v)| v.into_owned()).collect();
        if values.len() != 1 { return Err(StatusCode::FORBIDDEN); }
        Ok(values[0].clone())
    };
    let allowed = match (method.as_str(), path.as_str()) {
        ("POST", "/mcp" | "/messages") | ("GET", "/sse" | "/api/catalog" | "/api/workspaces") => true,
        ("POST", "/api/search" | "/api/web/search") => true,
        ("GET", "/api/history/searches") => workspace_allowed(grant, &query("workspace_id")?, false),
        ("GET", "/api/citations") => paper_allowed(&state.db, grant, &query("id")?),
        _ if parts.len() == 5 && parts[1] == "api" && parts[2] == "workspaces" && parts[4] == "papers" =>
            ["GET","POST","PATCH","DELETE"].contains(&method.as_str()) && workspace_allowed(grant, parts[3], method != Method::GET),
        _ if method == Method::GET && parts.get(1..3) == Some(&["api", "paper"][..]) => {
            let id = if path.ends_with("/citations") { path.trim_start_matches("/api/paper/").trim_end_matches("/citations") } else { path.trim_start_matches("/api/paper/") };
    ...
    if method == Method::POST && (path == "/api/search" || path.ends_with("/papers")) {
        ...
        let bytes = axum::body::to_bytes(body, 1024 * 1024).await.map_err(|_| StatusCode::PAYLOAD_TOO_LARGE)?;
        let mut value: serde_json::Value = serde_json::from_slice(&bytes).map_err(|_| StatusCode::BAD_REQUEST)?;
        if path == "/api/search" {
            if value.get("workspace_id").is_some_and(|id| !id.is_null() && !id.as_str().is_some_and(|id| workspace_allowed(grant, id, false)))
                || ["searxng_url", "searxng_categories", "searxng_engines"].iter().any(|key| value.get(*key).is_some_and(|v| !v.is_null())) {
                return Err(StatusCode::FORBIDDEN);
            }
        } else {
            let id = value["paper"]["id"].as_str().ok_or(StatusCode::BAD_REQUEST)?;
            if !paper_allowed(&state.db, grant, id) { return Err(StatusCode::FORBIDDEN); }
            if let Some(paper) = state.db.find_paper(id) { value["paper"] = serde_json::to_value(paper).unwrap(); }
        }
        return Ok(Request::from_parts(parts, axum::body::Body::from(value.to_string())));
    }
    Ok(request)
}
...
mod tests {
    ...
    async fn gateway_token_protects_rest_and_mcp() {
        let state = state();
        state.db.set_config("mcp_auth_token", "0123456789abcdef").unwrap();
        let app = crate::server::gateway_router(state);
        // Sanitized config stays readable without a token for bootstrap.
        let request = Request::builder().uri("/api/config").header("host", "127.0.0.1:8795").body(Body::empty()).unwrap();
        assert_eq!(app.clone().oneshot(request).await.unwrap().status(), StatusCode::OK);
        let body = r#"{"jsonrpc":"2.0","id":1,"method":"tools/list"}"#;
        for (auth, expected) in [(None, StatusCode::UNAUTHORIZED), (Some("Bearer 0123456789abcdef"), StatusCode::OK)] {
            let mut request = Request::builder().method("POST").uri("/mcp").header("host", "127.0.0.1:8795").header("content-type", "application/json");
            if let Some(value) = auth { request = request.header("authorization", value); }
            let request = request.body(Body::from(body)).unwrap();
            assert_eq!(app.clone().oneshot(request).await.unwrap().status(), expected, "auth={auth:?}");
        }
        // SSE legacy transport is protected too.
        let request = Request::builder().uri("/sse").header("host", "127.0.0.1:8795").body(Body::empty()).unwrap();
    ...
    async fn production_router_enforces_cors_and_blocks_config_mutation() {
        ...
        let app = crate::server::gateway_router(crate::server::AppState { db: db.clone(), engine: std::sync::Arc::new(crate::engine::AcademicEngine::new()), port: 8795, mcp_sessions: Default::default() });
        let request = Request::builder().method("POST").uri("/api/config")
            .header("host", "127.0.0.1:8795").header("origin", "https://evil.example")
            .header("content-type", "application/json").body(Body::from(r#"{"domain_preset":"medical"}"#)).unwrap();
        ...
        let request = Request::builder().method("OPTIONS").uri("/api/config")
            .header("host", "127.0.0.1:8795").header("origin", "http://localhost:1420")
            .header("access-control-request-method", "POST").header("access-control-request-headers", "content-type")
            .body(Body::empty()).unwrap();
        let response = app.clone().oneshot(request).await.unwrap();
        assert!(response.status().is_success());
        assert_eq!(response.headers()["access-control-allow-origin"], "http://localhost:1420");
        let request = Request::builder().uri("/api/config").header("host", "localhost:8795")
            .header("origin", "http://localhost:1420").body(Body::empty()).unwrap();
        assert_eq!(app.oneshot(request).await.unwrap().status(), StatusCode::OK);
    }

    #[tokio::test]
    async fn blocks_foreign_origins_and_rebinding_but_allows_local_agents() {
        let app = Router::new().route("/api/config", get(|| async { "private settings" }))
        ...
        for (host, origin, fetch_site, expected) in [
            ...
            ("localhost:8795", Some("http://localhost:1420"), Some("same-site"), StatusCode::OK),
            ("localhost:8795", Some("tauri://localhost"), None, StatusCode::OK),
            ("localhost:8795", Some("https://tauri.localhost"), None, StatusCode::OK),
            ("localhost:8795", Some("https://evil.example"), None, StatusCode::FORBIDDEN),
            ("localhost:8795", Some("null"), None, StatusCode::FORBIDDEN),
            ("localhost:8795", Some("http://localhost:9999"), None, StatusCode::FORBIDDEN),
            ("evil.example:8795", None, None, StatusCode::FORBIDDEN),
            ("localhost:8795", None, Some("cross-site"), StatusCode::FORBIDDEN),
        ] {
            ...
            if let Some(site) = fetch_site { request = request.header("sec-fetch-site", site); }
    ...
    async fn rejections_carry_cors_headers_for_the_trusted_ui_only() {
        ...
        let request = Request::builder().uri("/api/agents").header("host", "localhost:8795")
            .header("origin", "http://localhost:1420").body(Body::empty()).unwrap();
        let response = app.clone().oneshot(request).await.unwrap();
        ...
        assert!(value["error"].as_str().unwrap().contains("administrator token"), "{value}");
